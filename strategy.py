...
def add_volatility_regime(df, fast=None, slow=None):
    ...
    # Same series as atr_slow, just shifted - reuse rather than recompute so the two
    # can never drift apart if the window/min_periods is tuned on only one of them.
    df["baseline_atr"] = atr_slow.shift(1)
    return df
