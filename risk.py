...
def calculate_trade(account_balance, entry_price, atr, direction, epic=None, quote_to_account_rate=1.0,
    ...
    # The account is denominated in CAD but every metal instrument is quoted in USD, so
    # the risk budget and the stop distance are in different currencies. Convert the
    # budget into the quote currency before dividing, otherwise size comes out over by
    # the full exchange rate (~39% at USD/CAD 1.39).
...
if __name__ == "__main__":
    ...
    # Currency conversion: a CAD budget against a USD-quoted instrument must size
    # smaller than the naive same-currency calculation, by exactly the rate.
    same = calculate_trade(1000, 4332.0, 16.0, "LONG", epic="GOLD", quote_to_account_rate=1.0)
    conv = calculate_trade(1000, 4332.0, 16.0, "LONG", epic="GOLD", quote_to_account_rate=1.389)
    assert conv["size"] < same["size"], "CAD conversion should reduce size vs unconverted"
    print(f"PASS: CAD conversion  unconverted={same['size']} converted={conv['size']}")

    # Minimum-size guard: a balance too small for the instrument must raise, not
    # silently produce a zero-unit order.
    try:
        calculate_trade(10, 4332.0, 16.0, "LONG", epic="SILVER", quote_to_account_rate=1.389)
